...
bool is_valid_faultAddr(void *fault_addr);
...
page_fault (struct intr_frame *f) 
{
 ...
 /* if(!is_valid_faultAddr(fault_addr)){
	printf("valid fault address----------------\n");
	sys_exit(-1);
  }*/
  //if(user)
	//sys_exit(-1);
  ...
  bool load=false;	
 // printf("inside page_fault %x and not_presetn=%d\n",pg_round_down(fault_addr),not_present);
	
  if(not_present)
  {
		//Check the faulting address in Supplementary table
		struct SPT *spt = spt_find(fault_addr);
		//printf("inside page_fault %x\n",fault_addr);
		if(spt != NULL)
		{
	//		printf("SPT is not null \n");
			load = page_load(spt);
		//	printf("page load from file is %d\n",load);
		}
		else if(is_stack_access(f->esp,fault_addr))
		{
			load = stack_growth(fault_addr);
		//	printf("page load from stack is %d\n",load);
		}
  }
  //printf("***************************************************************\n");
  if(!load)
  {
		//printf("Exiting because of Exception ***********************************************************\n");
		sys_exit(-1);
